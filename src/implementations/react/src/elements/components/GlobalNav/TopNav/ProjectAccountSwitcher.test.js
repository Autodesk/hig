...
describe("<ProjectAccountSwitcher>", () => {
  describe("configuring the flyout", () => {
    ...
    describe("with just one account", () => {
      beforeEach(() => {
        const accounts = [{ id: "1", label: "My account" }];
        wrapper = mount(<Context accounts={accounts} />);
      });

      it("does not show the caret", () => {
        expect(wrapper.find(ProjectAccountSwitcherAdapter)).not.toHaveProp(
          "showCaret"
        );
      });
    ...
    describe("with just one project", () => {
      beforeEach(() => {
        const projects = [{ id: "1", label: "My project" }];
    ...
    describe("with more than one account", () => {
      ...
      it("shows the caret", () => {
        expect(wrapper.find(ProjectAccountSwitcherAdapter)).toHaveProp(
          "showCaret",
          true
        );
      });
      ...
      it("passes click handlers", () => {
        expect(
          wrapper.find(ProjectAccountSwitcherAdapter).prop("onClick")
        ).toBeInstanceOf(Function);
        expect(
          wrapper.find(ProjectAccountSwitcherAdapter).prop("onClickOutside")
        ).toBeInstanceOf(Function);
      });
    });
  ...
  describe("showing the active item", () => {
    const accounts = [
      {
        id: "1",
        label: "Global Construction",
        image: "/images/accounts/1.png"
      },
    ...
    describe("with nothing active", () => {
      it("activates the first project, account, or both", () => {
        ...
        expect(wrapper.find(ProjectAccountSwitcherAdapter)).toHaveProp(
          "activeLabel",
          "Global Construction / General Hospital"
        );
        expect(wrapper.find(ProjectAccountSwitcherAdapter)).toHaveProp(
          "activeImage",
    ...
    describe("with an account active", () => {
      ...
      beforeEach(() => {
        wrapper = mount(
          <Context accounts={accounts} projects={[]} activeAccountId="1" />
        );
      });
    ...
    describe("with a project active", () => {
      ...
      beforeEach(() => {
        wrapper = mount(
          <Context accounts={[]} projects={projects} activeAccountId="1" />
        );
      });
  ...
  describe("passing props", () => {
    ...
    it("passes projects", () => {
      expect(wrapper.find(Project).at(0)).toHaveProp(
        "label",
        "General Hospital"
      );
      expect(wrapper.find(Project).at(1)).toHaveProp(
        "label",
        "Rinky-Dink Rollerrink"
      );
    });

    it("passes accounts", () => {
      expect(wrapper.find(Account).at(0)).toHaveProp(
        "label",
        "Global Construction"
      );
      expect(wrapper.find(Account).at(1)).toHaveProp(
        "label",
        "HDR Remodelling"
      );
    });
