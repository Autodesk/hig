...
class ProjectAccountSwitcher extends Component {
  ...
  activeProps() {
    const activeAccount = this.props.accounts.find(
      a => a.id === this.renderedActiveAccountId()
    );
    const activeProject = this.props.projects.find(
      a => a.id === this.renderedActiveProjectId()
    );
    ...
    if (activeAccount && activeProject) {
      ...
      return {
        activeImage: activeProject.image,
        activeLabel: activeProject.label,
        activeType: "project"
      };
    } else if (activeAccount) {
      return {
        activeImage: activeAccount.image,
  ...
  render() {
    const projects = this.props.projects.map(project => ({
      ...project,
      active: project.id === this.renderedActiveProjectId()
    }));
    const accounts = this.props.accounts.map(account => ({
      ...account,
      active: account.id === this.renderedActiveAccountId()
    }));
    ...
    return (
      ...
      >
        {accounts.map((a, i) => {
          if (i === 0) {
            return (
              <Account onClick={this.handleAccountClick} key={a.id} {...a} />
            );
          }
          return (
            <Account onClick={this.handleAccountClick} key={a.id} {...a} />
        ...
        {projects.map((p, i) => {
          if (i === 0) {
            return (
              <Project onClick={this.handleProjectClick} key={p.id} {...p} />
            );
          }
          return (
            <Project onClick={this.handleProjectClick} key={p.id} {...p} />
