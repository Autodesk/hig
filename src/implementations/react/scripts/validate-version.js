...
const remotePackageUrl =
  "https://raw.githubusercontent.com/Autodesk/hig/master/src/implementations/react/package.json";
...
function withRemoteVersion(successCallback, errorCallback) {
  request(remotePackageUrl, (error, response) => {
...
withRemoteVersion(
  ...
  (error, code) => {
    exitError(
      `Error retreiving remote version:\n` +
        `Error message: ${error}\n` +
        `Http code: ${code}`
    );
  }
