...
describe("<Tabs>", () => {
  ...
  it("renders tabs", () => {
    const { reactContainer } = setupInitialTabs();
  ...
  it("updates tab labels", () => {
    ...
    expect(
      reactContainer.querySelectorAll(".hig__global-nav__sub-nav__tabs__tab")[0]
        .textContent
    ).toEqual("Goodbye");
    expect(
      reactContainer.querySelectorAll(".hig__global-nav__sub-nav__tabs__tab")[1]
        .textContent
    ).toEqual("Foo");
  });
});
