import { match, filter, group, mergeState } from "./model";
...
describe("SideNav model", () => {
  ...
  const submoduleB = {
    id: "B",
    title: "Belive in your sumbodule",
    moduleId: "2"
  };
  ...
  describe("filter", () => {
    it("returns submodules matching the query", () => {
      ...
      expect(result.submodules).not.toEqual(
        expect.arrayContaining([submoduleB])
      );
    });
      ...
      expect(group({ modules, submodules })).toEqual([
        ...
        },
        {
          modules: [
            {
              id: "2",
              title: "2 legit to quit",
              groupId: "Y",
              submodules: [submoduleB]
            }
          ]
        }
      ]);
